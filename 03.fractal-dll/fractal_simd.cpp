...
extern "C" __stdcall // declarar la función con enlace C para evitar name mangling es decir que el compilador no cambie el nombre de la función para soportar sobrecarga de funciones, lo que permite que la función sea llamada desde otros lenguajes o desde código C sin problemas de enlace.
    void julia_simd(double x_min, double y_min, double x_max, double y_max, uint32_t width, uint32_t height, int max_iterations, uint32_t *pixel_buffer)
{
